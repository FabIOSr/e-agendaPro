-- Migration 30: criação atômica de agendamento
-- Centraliza as validações críticas no banco para evitar double-booking por corrida.
...
BEGIN
  ...
  IF v_motivo IS NOT NULL THEN
    RETURN jsonb_build_object(
      'ok', false,
      'erro', 'Horário indisponível: ' || v_motivo || '. Por favor, escolha outro horário.',
      'http_status', 409
    );
  END IF;
...
IS 'Cria agendamento de forma atômica com lock por prestador, validações de limite, conflitos, bloqueios e reserva da lista de espera.';
